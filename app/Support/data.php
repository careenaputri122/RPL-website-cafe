...
function create_reservation($data)
{
    ...
    if (using_database()) {
        ...
        try {
            ...
            // Combined payment
            $payStmt = $pdo->prepare('INSERT INTO payment (id_reservasi, id_pesanan, bukti_tf, status_payment, total) VALUES (?, ?, NULL, ?, ?)');
            $payStmt->execute([$resId, $orderId, 'pending', $totalPayment]);
            $payId = (int)$pdo->lastInsertId();
            
            $pdo->commit();
            $msg = $orderId ? 'Reservasi & Pre-order berhasil dibuat.' : 'Reservasi berhasil dibuat.';
            return ['ok' => true, 'id' => $resId, 'kode' => 'RSV-' . str_pad((string)(1000 + $resId), 4, '0', STR_PAD_LEFT), 'payment_id' => $payId, 'message' => $msg . ' Silakan lakukan pembayaran total ' . rupiah($totalPayment)];
        } catch (Throwable $e) {
            $pdo->rollBack();
    ...
    $_SESSION['payments'][] = [
        ...
        'nama' => $user['name'], 'total' => $totalPayment, 'bukti_tf' => 'Belum ada file', 'status' => 'pending'
    ];
